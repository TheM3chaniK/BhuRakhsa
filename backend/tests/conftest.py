...
def civilian_user() -> User:
    """Fixture returning a mock Civilian user (Citizen A)."""
    now = datetime.now(timezone.utc)
    return User(
        id=uuid.uuid4(),
        full_name="Citizen User A",
...
@pytest.fixture
def civilian_b_user() -> User:
    """Fixture returning a mock Civilian user (Citizen B)."""
    now = datetime.now(timezone.utc)
    return User(
        id=uuid.uuid4(),
        full_name="Citizen User B",
